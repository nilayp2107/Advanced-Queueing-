/* 
teqrawenq.c 
Description - Enqueue RAW message to the Transactional Event Queue

To run this .c file 
Step 1. Compile it $ loutl teqrawenq.c
...
int main(int argc, char *argv[])
{
  ...
  checkerr(errhp, OCITypeByName(envhp, errhp, svchp,
          ...
          (CONST text *)"RAW", (ub4)strlen("RAW"), (text *)0, 0,
  ...
  strcat(queuename, ".raw_queue_teq");

  /* Allocate the Raw descriptor */
  mesgp = 0;
  nmesgp = (OCIInd*) malloc(sizeof(OCIInd));
  memset(nmesgp, 0, sizeof(OCIInd));
  checkerr(errhp,OCIRawAssignBytes(envhp, errhp, (ub1 *)"Hello World", (ub2)
                             strlen("Hello World"), &mesgp));

  /* Enqueue the RAW message */
